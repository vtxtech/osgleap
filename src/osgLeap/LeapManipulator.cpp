...
namespace osgLeap {
	...
	LeapManipulator::LeapManipulator(const Mode& mode): osgGA::OrbitManipulator(), Leap::Listener(),
		...
		leftHandID_(0),
		rightHandID_(0),
		lastPositionLeftHand_(Leap::Vector(0.0f, 0.0f, 0.0f)),
		lastPositionRightHand_(Leap::Vector(0.0f, 0.0f, 0.0f)),
		handsDistance_(0.0f),
	...
	bool LeapManipulator::handle( const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& us )
	{
		if (ea.getEventType() == osgGA::GUIEventAdapter::FRAME) {
			...
			osg::Timer_t frameStamp = frameStamp_;

			if (frameStamp != lastFrameStamp_) {
				osg::Timer_t deltaTimeSinceLastFrame = frameStamp - lastFrameStamp_;
